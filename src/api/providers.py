...
# Serializes export generation per provider so two requests for the same
# (or different) provider can't build heavy exports in memory at the same
# time. This is what caused the concurrent magenta2 + magentaeu_at exports
# to double peak memory and trigger the OOM kill (exit 137).
_export_locks_guard = threading.Lock()
_export_locks: dict[str, threading.Lock] = {}


def _get_export_lock(provider_name: str) -> threading.Lock:
    with _export_locks_guard:
        lock = _export_locks.get(provider_name)
        if lock is None:
            lock = threading.Lock()
            _export_locks[provider_name] = lock
        return lock
...
def export_provider_epg_xml_gz(identifier):
    ...
    try:
        ...
        # Only one export per provider runs at a time; a second caller that
        # arrives while generation is in progress waits for the lock and
        # then re-checks the cache instead of duplicating the work below.
        export_lock = _get_export_lock(provider.name)
        with export_lock:
            # Re-check cache now that we hold the lock: another request may
            # have just finished generating it while we were waiting.
            if os.path.exists(cache_path):
                mtime = os.path.getmtime(cache_path)
                age_hours = (datetime.now(timezone.utc).timestamp() - mtime) / 3600
                if age_hours < 24 and not force:
                    logger.info(
                        f"Serving export for {provider.name} generated by a "
                        f"concurrent request ({age_hours:.1f}h old)"
                    )
                    return send_file(
                        cache_path,
                        mimetype="application/gzip",
            ...
            # Serialize to XMLTV, writing straight into the gzip stream.
            # serialize_tv_to_file() skips the tostring() -> minidom ->
            # toprettyxml() round trip that serialize_tv() does -- that
            # chain built the full document 2-3 extra times in memory
            # purely for cosmetic indentation, and was the primary driver
            # of the OOM kill on the magentaeu_at export. No full XML
            # string is ever materialized here.
            serializer = XMLTVSerializer()
            os.makedirs(export_dir, exist_ok=True)
            tmp_path = cache_path + ".tmp"
            try:
                with open(tmp_path, "wb") as raw_f:
                    with gzip.GzipFile(fileobj=raw_f, mode="wb", compresslevel=6) as gz_file:
                ...
                if os.path.exists(tmp_path):
                    os.remove(tmp_path)
                return jsonify({"error": f"Failed to generate export: {e}"}), 500
            ...
            # Stream from disk rather than holding the bytes in a Response
            # body — send_file uses Flask's chunked file wrapper instead of
            # loading the whole gzip into the werkzeug response object.
            response = send_file(
                cache_path,
                mimetype="application/gzip",
                as_attachment=True,
